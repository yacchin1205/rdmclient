...
from .utils import norm_remote_path, split_storage, makedirs, checksum_path
...
async def list_(args):
    ...
    async for store in project.storages:
        ...
        if base_provider is not None and base_provider != prefix:
            continue
        async for file_ in filter_by_path_pattern(store, base_file_path):
            if is_folder(file_):
                continue
            path = file_.path
            if path.startswith('/'):
